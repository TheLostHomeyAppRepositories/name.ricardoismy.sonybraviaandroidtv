...
class SonyBraviaAndroidTVCommunicator extends Homey.SimpleClass {
  ...
  //       'POST', 
  //       `${this.getProtocol(device)}${settings.ip}/sony/system`,
  //       {
  //         cache: 'no-cache',
  //         headers: {
  ...
  async sendCommand(device, command) {
    ...
    try {
      let response = await http.request( 
        'POST', 
        `${this.getProtocol(device)}${settings.ip}/sony/IRCC`,
        {
          cache: 'no-cache',
          headers: {
            'X-Auth-PSK': settings.psk,
  ...
  // HELPER ===================================================================================================================
  getProtocol(device) {
    return device.data.settings['https'] ? 'https://' : 'http://';
  }
  ...
  async getVolume(device, target='speaker') {
    ...
    try {
      ...
      response = await http.request( 
        'POST', 
        `${this.getProtocol(device)}${settings.ip}/sony/audio`,
        {
          cache: 'no-cache',
          headers: {
            'X-Auth-PSK': settings.psk,
    ...
    let state = response.result[0].filter((e => e.target === 'speaker'))[0];
    if (state){ 
      return state;
    }
    else{
  ...
  async getApps(device) {
    ...
    try {
      ...
      response = await http.request( 
        'POST', 
        `${this.getProtocol(device)}${settings.ip}/sony/appControl`,
        {
          cache: 'no-cache',
          headers: {
            'X-Auth-PSK': settings.psk,
  ...
  async getCurrentExternalInputs(device) {
    ...
    try {
      ...
      response = await http.request( 
        'POST', 
        `${this.getProtocol(device)}${settings.ip}/sony/avContent`,
        {
          cache: 'no-cache',
          headers: {
            'X-Auth-PSK': settings.psk,
  ...
  async getPictureQualitySettings(device, type='') {
    const settings = device.getSettings();
    let response;
    try {
      let body = JSON.stringify({
      ...
      response = await http.request( 
        'POST', 
        `${this.getProtocol(device)}${settings.ip}/sony/video`,
        {
          cache: 'no-cache',
          headers: {
            'X-Auth-PSK': settings.psk,
    ...
    let modes = response.result[0];
    if (modes){ 
      return modes;
    }
    else{
      throw new Error("PictureQualitySettings not available");
    }
  }

  async setPictureQualitySettings(device, type, value) {
    const settings = device.getSettings();
    let response;
    try {
      let body = JSON.stringify({
      ...
      // device.log(`An error occured on setPictureQualitySettings: `, err.message);
      throw err;
    }
    if (response.error){
      throw new Error(response.error);
